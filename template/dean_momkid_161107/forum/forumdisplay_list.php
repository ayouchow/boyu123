								...
								<tr class="deanactions fadeIn">
									...
									<th class="$thread[folder] forumtit">
                                        ...
                                        <div class="deanthreads_summary">
                                        	<!--{eval $summary = DB::result(DB::query("SELECT message FROM ".DB::table('forum_post')." WHERE tid = '$thread[tid]' AND first = '1' ")); $summary_echo = preg_replace ("/\[[a-z][^\]]*\]|\[\/[a-z]+\]/i",'',preg_replace("/\[attach\]\d+\[\/attach\]/i",'',$summary));}-->
                                                $summary_echo
                                        </div>

                                        <div class="deanforumlist_pics">
                                        	<!--{eval $thread_id = substr($thread[tid], -1); $yuerbaike = DB::fetch_all("SELECT * FROM ".DB::table('forum_attachment_'.$thread_id.'')." WHERE tid = '$thread[tid]' AND isimage = '1' ORDER BY `aid` ASC LIMIT 5");}-->
						...
						<ul id="waterfall" class="ml waterfall cl deanwaterfall">
							...
							<li style="width:{$waterfallwidth}px">
								...
								<h5 class="deanwaterfalltitle">
                                    ...
                                    <a class="deanwft" href="forum.php?mod=viewthread&tid=$thread[tid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra"$thread[highlight]{if $thread['isgroup'] == 1 || $thread['forumstick']} target="_blank"{else} onclick="atarget(this)"{/if} title="$thread[subject]">$thread[subject]</a>
                                    <div class="clear"></div>
								</h5>
                                <div class="deanwaterfallinfo">
                                        
                                    <em class="deanviews">$thread[views]</em>
                                    <em class="deancomments">$thread[replies]</em>
                                    <em class="deanrecommends">$thread[recommends]</em>
                                </div>
								<div class="waterfallbottom">
                                	<!--{hook/forumdisplay_author $key}-->
									<!--{if $thread['authorid'] && $thread['author']}-->
										<a class="deanlistavator" href="home.php?mod=space&uid=$thread[authorid]">
                                        <!--{if $thread['authorid'] && $thread['author']}-->
									...
									<!--{/if}-->
                                    <div class="deanftsj">$thread[dateline]</div>
